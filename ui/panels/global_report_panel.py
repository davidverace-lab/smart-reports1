...
class GlobalReportPanel(ctk.CTkFrame):
    ...
    def _show_preview_text(self, module, data, total_population):
        ...
        for record in data:
            ...
            table_rows.append(
                f"  │ Módulo {record['modulo']}  │   {record['poblacion_total']:<6} │  {record['registrado']:<5} │  {record['en_proceso']:<5}  │  {record['completado']:<5}   │"
            )
...
└─────────────────────────────────────────────────────────────────────────────┘
  ...
  ┌─────────────┬──────────┬──────────┬────────────┬─────────────┐
  │   Módulo    │ Población│Registrado│ En Proceso │ Completado  │
  │             │  Total   │          │            │             │
  ├─────────────┼──────────┼──────────┼────────────┼─────────────┤
